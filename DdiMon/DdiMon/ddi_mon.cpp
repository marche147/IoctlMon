	...
	*/
	{
		...
		DdimonpHandleNtDeviceIoControlFile, nullptr,
	}
};
...
template <typename T>
T DdimonpFindOrignal(T handler) {
...
_Use_decl_annotations_ static NTSTATUS DdimonpHandleNtDeviceIoControlFile(
	__in HANDLE FileHandle,
	__in_opt HANDLE Event,
	__in_opt PIO_APC_ROUTINE ApcRoutine,
	__in_opt PVOID ApcContext,
	...
	return IoctlMonNtDeviceIoControlFileHook(reinterpret_cast<PVOID>(original), FileHandle, Event, ApcRoutine, ApcContext, IoStatusBlock, IoControlCode, InputBuffer, InputBufferLength, OutputBuffer, OutputBufferLength);
}